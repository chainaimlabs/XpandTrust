...
impl<C: GKRFieldConfig> Circuit<C> {
    ...
    pub fn load_witness_bytes(
        ...
        println!("{}", self.log_input_size());
        let private_input_size = 1 << self.log_input_size();
        //   let private_input_size = 1;
        ...
        if witness.num_witnesses < desired_number_of_witnesses {
            ...
            witness.values.truncate(desired_number_of_witnesses * total_size);
            witness.num_witnesses = desired_number_of_witnesses;
        }
    ...
    pub fn prover_process_witness(&mut self, witness: Witness<C>, mpi_config: &MPIConfig) {
        ...
        let input =
            &witness.values
                [
                    rank * total_size * C::get_field_pack_size()..(rank + 1) *
                        total_size *
        ...
        for i in 0..public_input_size {
            let mut public_wit_i: Vec<<C as GKRFieldConfig>::CircuitField> = vec![];
            for j in 0..C::get_field_pack_size() {
    ...
    pub fn verifier_process_witness(&mut self, witness: Witness<C>, mpi_config: &MPIConfig) {
        ...
        for i_rank in 0..mpi_config.world_size() {
            let input =
                &witness.values
                    [
                        i_rank * total_size * C::get_field_pack_size()..(i_rank + 1) *
                            total_size *
                            C::get_field_pack_size()
                    ];
...
impl<C: GKRFieldConfig> Circuit<C> {
    ...
    pub fn evaluate(&mut self) {
        for i in 0..self.layers.len() - 1 {
            ...
            layer_p_1.last().unwrap().evaluate(&mut layer_p_2[0].input_vals, &self.public_input);
            log::trace!(
                "layer {} evaluated - First 10 values: {:?}",
                i,
                self.layers[i + 1].input_vals.iter().take(10).collect::<Vec<_>>()
            );
        }
        let mut output = vec![];
        ...
        log::trace!(
            "First ten values: {:?}",
            self.layers.last().unwrap().output_vals.iter().take(10).collect::<Vec<_>>()
        );
    }
